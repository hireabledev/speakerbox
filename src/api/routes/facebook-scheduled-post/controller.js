...
export async function create(req) {
  ...
  const post = req.app.models.FacebookScheduledPost.build({ date });
  const job = await req.app.addJob({
    type: JOB_TYPE,
    title: `Share on Facebook ${post.id}`,
    delay: date,
    data: {
      ...req.body,
...
export async function update(req) {
  const post = await req.app.models.FacebookScheduledPost
...
export async function remove(req) {
  const share = await req.app.models.FacebookScheduledPost
    .scopeForUserAccounts(req.user, req.query.user)
    .findByIdOr404(req.params.id);
