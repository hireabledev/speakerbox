...
export async function create(req) {
  ...
  const post = req.app.models.LinkedinScheduledPost.build({ date });
...
export async function update(req) {
  const post = await req.app.models.LinkedinScheduledPost
...
export async function remove(req) {
  const share = await req.app.models.LinkedinScheduledPost
    .scopeForUserAccounts(req.user, req.query.user)
    .findByIdOr404(req.params.id);
